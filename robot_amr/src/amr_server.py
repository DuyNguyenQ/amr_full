...
def get_goal_position(choice):
    ...
    #  (x, y, theta, priority)
    mang_toa_do_goal = [(0.0, 0.0, -1.57, 0), (0.0, 0.3, 0, 1), (0.0, 0.4,0, 2), (3.0, 0.0, 0, 3), (4.0, 0.0, 0, 4), (5.0, 0.0, 0, 5),   # 0 --> 5
                         (6.0, 0.0, 0, 6), (7.0, 0.0, 0, 7), (8.0, 0.0, 0, 8), (9.0, 0.0, 0, 9), (10.0, 0.0, 0, 10)]  # 6 --> 10
    if 0 <= choice <= 10:
...
def add_task_to_queue(choice):
    ...
    if goal is not None:
        ...
        if goal not in list(task_queue.queue):
            task_queue.put(goal)
            rospy.loginfo("Cong viec da duoc them vao hang cho")
        else:
            
            pass
        
    # Sap xep theo priority        
    sorted_tasks = sorted(list(task_queue.queue), key=lambda x: (x[3]))
...
def main():
    global user_choice, choice_continue,send
